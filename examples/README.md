...
- `resume-thread.ts` - Resume an existing thread by ID (via onSessionCreated callback).
- `capture-session-id.ts` - Capture sessionId from finish event's providerMetadata for persistence.
